...
-- 2. Clients table (under businesses)
...
-- 3. Integration keys (now with business_id reference)
CREATE TABLE integration_keys (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  business_id TEXT REFERENCES businesses(business_id) ON DELETE CASCADE,
  key_type TEXT NOT NULL, -- 'api', 'stripe', 'fly', 'supabase'
  key_name TEXT, -- e.g., 'production', 'staging', 'my-fly-app'
...
-- 4. Single metrics table (business + client + machine)
...
-- 5. Plans table (for plan limits and billing)
CREATE TABLE plans (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  business_id TEXT REFERENCES businesses(business_id) ON DELETE CASCADE,
  plan_name TEXT NOT NULL, -- 'free', 'pro', 'enterprise'
  rate_limits JSONB, -- Default rate limits for this plan
  pricing JSONB, -- Pricing rules
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 6. Plan limits table (metric-specific limits)
CREATE TABLE plan_limits (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  business_id TEXT REFERENCES businesses(business_id) ON DELETE CASCADE,
  metric_name TEXT NOT NULL,
  limit_value FLOAT NOT NULL,
  limit_period TEXT NOT NULL, -- 'daily', 'monthly', 'yearly'
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  UNIQUE(business_id, metric_name, limit_period)
);

-- 7. Breach thresholds (for auto-scaling & alerts)
CREATE TABLE breach_thresholds (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  business_id TEXT REFERENCES businesses(business_id) ON DELETE CASCADE,
  client_id TEXT REFERENCES clients(client_id) ON DELETE CASCADE, -- Optional: client-specific thresholds
  machine_id TEXT, -- Optional: machine-specific thresholds
  metric_name TEXT NOT NULL,
  threshold_value FLOAT NOT NULL,
  threshold_operator TEXT NOT NULL, -- 'gt', 'lt', 'eq', 'gte', 'lte'
  webhook_url TEXT,
  integration_key_id UUID REFERENCES integration_keys(id), -- Reference to Fly integration
  scaling_config JSONB, -- Machine specs, regions, etc.
  is_active BOOLEAN DEFAULT TRUE,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  UNIQUE(business_id, client_id, machine_id, metric_name)
);
...
CREATE INDEX idx_breach_thresholds_business ON breach_thresholds(business_id);
CREATE INDEX idx_breach_thresholds_client ON breach_thresholds(client_id);
CREATE INDEX idx_breach_thresholds_machine ON breach_thresholds(machine_id);
CREATE INDEX idx_breach_thresholds_active ON breach_thresholds(is_active);
